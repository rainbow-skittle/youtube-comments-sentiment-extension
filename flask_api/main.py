...
def load_model_and_vectorizer(model_name, model_version, vectorizer_path):
    ...
    mlflow.set_tracking_uri("http://ec2-18-215-169-8.compute-1.amazonaws.com:5000/")  # Replace with your MLflow tracking URI
    client = MlflowClient()
...
def predict_with_timestamps():
    try:
        # Force JSON parsing (avoids issues with invalid headers)
    ...
    if not comments_data or not isinstance(comments_data, list):
        return jsonify({"error": "No comments provided or wrong format"}), 400
    ...
    try:
        ...
        df = pd.DataFrame(dense_comments, columns=feature_names) 
        # Make predictions 
...
def predict():
    ...
    # Return response with original comments and predicted sentiments
    response = [
